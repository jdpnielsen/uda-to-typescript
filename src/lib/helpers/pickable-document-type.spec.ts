...
describe('getPickableTypes', () => {
	it('Should count root document as pickable', async () => {
		const artifacts = await collectArtifacts('./src/__tests__/__fixtures__/v17/document-type__8856c80b6f524f87969a863a61eaa6aa.uda');
	...
	it('Should not count non-root document as pickable', async () => {
		const artifacts = await collectArtifacts('./src/__tests__/__fixtures__/v17/document-type__5f471833166846a4a2fbf04c1a3946a8.uda');
		const documentTypes = Array.from(artifacts['document-type'].values());
